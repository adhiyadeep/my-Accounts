...
/**
 * Generated class for the IncomemodeWisePage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-incomemode-wise',
  templateUrl: 'incomemode-wise.html',
})
export class IncomemodeWisePage {
  incomeData: any;
  fromDt: any;
  toDt: any;
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad IncomemodeWisePage');
  ...
  filterData() {
    //Cash
    this.cashFilter = this.incomeData.filter(res => res.paymentmode == "Cash")
    this.cashFilter.forEach((e: any) => {
      this.cashTotal = this.cashTotal + Number(e.amount);
    });

    //Cheque
    this.chequeFilter = this.incomeData.filter(res => res.paymentmode == "Cheque")
    this.chequeFilter.forEach((e: any) => {
      this.chequeTotal = this.chequeTotal + Number(e.amount);
    });

    //Credit Card
    this.cCardFilter = this.incomeData.filter(res => res.paymentmode == "Credit Card")
    this.cCardFilter.forEach((e: any) => {
      this.cCardTotal = this.cCardTotal + Number(e.amount);
    });

    //Debit Card
    this.dCardFilter = this.incomeData.filter(res => res.paymentmode == "Debit Card")
    this.dCardFilter.forEach((e: any) => {
      this.dCardTotal = this.dCardTotal + Number(e.amount);
    });

    //Net Banking
    this.netBankingFilter = this.incomeData.filter(res => res.paymentmode == "Net Banking")
    this.netBankingFilter.forEach((e: any) => {
      this.netBankingTotal = this.netBankingTotal + Number(e.amount);
    });

    //Others
    this.othersFilter = this.incomeData.filter(res => res.paymentmode == "Net Banking")
    this.othersFilter.forEach((e: any) => {
      this.othersTotal = this.othersTotal + Number(e.amount);
    });

    //Paytm
    this.paytmFilter = this.incomeData.filter(res => res.paymentmode == "Paytm")
